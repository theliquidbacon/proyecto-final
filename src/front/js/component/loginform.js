...
import introvertidoslogin from "../../img/introvertidoslogin.png";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const userIsintrovert = data.is_introvert;

        localStorage.setItem("miTokenJWT", token);
        localStorage.setItem("loggedUserEmail", email);
        localStorage.setItem("loggedUsername", username);
        localStorage.setItem("isintrovert", userIsintrovert);

        setEmail('');
        setPassword('');
        setUsername(username);
        setIsintrovert(userIsintrovert);
        navigate('/home');
        handleLogin();
      } else {
      ...
      <div className="container-form">
        <div className="row">
          ...
          <div className="col-md-6">
            <img src={introvertidoslogin} alt="introvertidos Login" />
          </div>
        </div>
      </div>
    </div>
